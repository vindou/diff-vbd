...
| pocket characterization (masking bias) | IN PROGRESS | 135-point grid (mu x indentation x kappa), 5 contexts (traced eager/vmap, each ± block-Jacobi, host) running | — |
| block-Jacobi preconditioned CG | DONE (code + gates) | per-vertex 3x3 static-Hessian blocks (elastic incident + exact barrier), PSD-floored, damped like the operator, rebuilt per pass; optional on both CG calls, default off, host driver untouched; prototype: 5/16 pocket stalls -> 0/16 at tol=1e-9 (pockets shuffle, not vanish, at 1e-11); preconditioned agreement sweep + preconditioned adjoint-vs-FD sweep green; full before/after stall table pending characterization | this commit |
| caller contract | NOT STARTED | max_iterations economics under vmap, read `converged`, masking guidance per characterization | — |
